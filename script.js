...
if(productCategories) {
    // Listen to click on product categories
    for(let i = 0; i < productCategories.children.length; i++) {
...
const createProductHtml = (index) => {
    ...
    newProductContainer.innerHTML =
        `<div class="main-product" data-id="${productArray[index].id}" data-category="${productArray[index].category}"><img src="${productArray[index].image}"
        class="product-image"
        alt="product image"
        />
    ...
    for(let i = 0; i < productArray.length; i++) {
        ...
        switch (selectedCategory) {
            ...
            case "Headsets":
                ...
                newProductContainer.children[0].getAttribute("data-category") === "headset" &&
                categoryContainer.insertAdjacentElement("beforeend", newProductContainer)
                break;
            case "Keyboards":
                newProductContainer.children[0].getAttribute("data-category") === "keyboard" &&
                categoryContainer.insertAdjacentElement("beforeend", newProductContainer)
                break;
            case "Mouse":
                newProductContainer.children[0].getAttribute("data-category") === "mouse" &&
                categoryContainer.insertAdjacentElement("beforeend", newProductContainer)
                break;
            case "Microphones":
                newProductContainer.children[0].getAttribute("data-category") === "microphone" && 
                categoryContainer.insertAdjacentElement("beforeend", newProductContainer)
                break;
        };
    };
...
const cartModalHistory = (id, quantity) => {
    ...
    modalCartProductContainer.innerHTML = 
    `<div data-id=${id} class="cart-modal-product">
        <img
        ...
        <div class="cart-modal-product-info">
            ...
            <h4>Price: <span>$</span><span>${(productArray[id].price)*quantity}</span></h4>
    ...
    if(itemExist) {
        ...
        for(let i = 0; i < cartModal.children.length - 3; i ++) {
            if(cartModal.children[i].firstChild.getAttribute("data-id") === id) {
                ...
                let accumulatedProductPrice = targetedProductContainer.lastElementChild.lastElementChild.previousElementSibling.lastElementChild
                const pricePerUnit = productArray[itemExist.id].price
                const newPrice = parseInt(accumulatedProductPrice.textContent) + (pricePerUnit * quantity)
                const selectOptionsElement = targetedProductContainer.lastElementChild.firstElementChild.nextElementSibling.lastElementChild
                const selectedOptionIndex = selectOptionsElement.options.selectedIndex
                selectOptionsElement.innerHTML = updateSelectedOption((selectedOptionIndex + 1) + quantity)
...
const updateProductChanges = (e, selectedIndex) => {
    ...
    e.target.parentElement.nextElementSibling.lastElementChild.innerText = newPrice;
    // Update total price
...
const deleteItem = (button) => {
    ...
    const selectedProductPrice = button.previousElementSibling.lastElementChild.textContent
...
cartModalIcon.addEventListener("click", (e) => {
    ...
    const continueShoppingModalIcon = document.querySelector(".continue-shopping")
